// Package files provides file management for d3 project files
package files
...
// Paths contains the standard paths for an d3 project
type Paths struct {
...
// GetPaths returns the standard paths for an d3 project
...
// GetD3DirPath returns the calculated .d3 directory path for the service.
func (s *Service) GetD3DirPath() string {
	if s == nil {
		return ""
	}
	return s.paths.D3Dir
}
...
// InitWorkspace initializes the d3 workspace
...
func (s *Service) InitWorkspace(clean bool) (string, []string, error) {
	// If clean, remove existing d3 directory
	if clean {
		if err := os.RemoveAll(s.paths.D3Dir); err != nil {
			return "", nil, fmt.Errorf("failed to clean d3 directory: %w", err)
		}
	}
	...
	sessionFile := filepath.Join(s.paths.D3Dir, "session.yaml")
	sessionData := struct {
	...
	message := "Initialized d3 in current workspace"
	if clean {
		message = "Clean initialized d3 in current workspace"
	}
...
func (s *Service) GenerateCoreRuleFile(feature, phase string) error {
	...
	d3Dir := filepath.Join(s.paths.CursorRulesDir, "d3")
...
// EnsureProjectFiles ensures the base project files exist in the d3 root
...
func (s *Service) EnsureProjectFiles(clean bool) ([]string, error) {
	...
	for _, filename := range projectFiles {
		filePath := filepath.Join(s.paths.D3Dir, filename)
