...
func TestStorage_Load(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMocks: func(mockFS *portsmocks.MockFileSystem) {
				mockFS.EXPECT().Stat(sessionFilePath).Return(testutil.MockFileInfo{}, nil).Times(1) // Use testutil.MockFileInfo
		...
		},
		{
			...
			wantState: &SessionState{CurrentFeature: "my-feat", CurrentPhase: Design, Version: "1.1"}, // Note: LastModified is zero value
			wantErr:   false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Compare states (ignoring time.Time for simplicity in basic comparison)
			if !tt.wantErr {
				if gotState == nil || tt.wantState == nil {
					...
					gotState.CurrentPhase != tt.wantState.CurrentPhase ||
...
func TestStorage_Save(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			stateToSave: &SessionState{CurrentFeature: "feat", CurrentPhase: Define},
			setupMocks: func(mockFS *portsmocks.MockFileSystem, state *SessionState) {
		...
		},
		{
			...
			setupMocks: func(mockFS *portsmocks.MockFileSystem, state *SessionState) {
				...
				// Expect WriteFile with marshalled data
		...
		},
		{
			...
			stateToSave: &SessionState{CurrentFeature: "feat-ok", CurrentPhase: Deliver, Version: "1.2"},
			setupMocks: func(mockFS *portsmocks.MockFileSystem, state *SessionState) {
				mockFS.EXPECT().MkdirAll(dirOfSessionFile, os.FileMode(0755)).Return(nil).Times(1)

				// Expect WriteFile and verify the marshalled content (partially)
				mockFS.EXPECT().WriteFile(sessionFilePath, gomock.Any(), os.FileMode(0644)).DoAndReturn(func(path string, data []byte, perm os.FileMode) error {
					// Basic check to see if marshalled data contains expected fields
					...
					if writtenState.CurrentFeature != state.CurrentFeature || writtenState.CurrentPhase != state.CurrentPhase || writtenState.Version != state.Version {
